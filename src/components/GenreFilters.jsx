  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {list.map(g => (
          <button key={g} onClick={()=>onSelect && onSelect(g)} className="rounded-full border border-slate-700/70 px-3 py-1 text-[11px] text-slate-300 hover:border-emerald-400/70 hover:text-emerald-300">
            {g}
          </button>
        ))}
      </div>
