...
export function ProfileShareModal({ open, onClose, profileType='producer', profileId, displayName }) {
  ...
  const rawId = profileId ? String(profileId) : ''

  let url = `${origin}/producer/${slug || rawId}`
  if (profileType === 'service') {
    url = `${origin}/services/${slug || rawId}`
  }
  if (profileType === 'beat') {
    url = `${origin}/beat/${slug || rawId}`
  }
...
function InstagramIcon(props){return(<img src="/assets/social/instagram.png" alt="" aria-hidden="true" {...props} />)}
function TikTokIcon(props){return(<img src="/assets/social/tiktok.png" alt="" aria-hidden="true" {...props} />)}
